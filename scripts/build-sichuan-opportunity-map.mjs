...
const DEFAULT_OUTPUT_PATH = path.resolve(process.cwd(), "research", "sichuan-opportunity-map.json");
const DEFAULT_CACHE_PATH = path.resolve(process.cwd(), "research", "sichuan-geocode-cache.json");
const DEFAULT_AGGREGATE_PATH = path.resolve(process.cwd(), "research", `四川省-产业与机会总览-${new Date().toISOString().slice(0, 10)}.json`);
...
async function geocode(address, city, cache) {
  ...
  const value = {
    longitude: Number.parseFloat(loc[0]),
    latitude: Number.parseFloat(loc[1]),
...
async function geocodeWithFallback(addresses, city, cache) {
  ...
  for (const address of addresses) {
    if (!address) continue;
    try {
      return await geocode(address, city, cache);
    } catch (error) {
      lastError = error;
    }
  }
...
function pickBattlePlan(item, county) {
  ...
  return {
    ...
    entry: [
      "先确认项目或政策当前处于立项、采购、招标还是实施阶段。",
      `优先联系 ${(item.targetOrgs || []).slice(0, 2).join("、") || `${county.city}${county.county}相关主管部门`}，不要只盯公告标题。`,
      `把可售服务拆成 ${services.join(" / ")}，形成标准方案。`,
    ],
    plan: [
      "建立县域线索台账，补齐预算、甲方、时间节点和阶段字段。",
      "把入驻商家能力贴到这条线索上，先筛出最匹配的 3 家。",
      "按 7 天节奏推进首轮触达、需求确认、报价和二次跟进。",
    ],
    action: [
      `先打电话或拜访 ${(item.targetOrgs || [county.county])[0]}。`,
      "同步整理同县近 12 个月相似项目，反推真实成交体量。",
      "把平台收费切口放在精准线索、撮合服务和履约协同。",
    ],
    monetization: `优先卖 ${services.slice(0, 2).join("、")} 这类更容易快速成交的服务，再延伸到更大客单价合作。`,
    risks: [
      "如果只展示线索、不做商家匹配和跟进台账，地图价值会迅速衰减。",
      `${type} 类机会通常周期长，需要持续推进而不是一次性查看。`,
    ],
  };
}
...
async function main() {
  ...
  const countyStatsByKey = new Map(
    (aggregate.countyStats || []).map((item) => [`${item.city}::${item.county}`, item]),
  );
  ...
  for (const payload of countyPayloads) {
    ...
    try {
      ...
      countyCenters.set(key, {
        longitude: Number((cityCenter.longitude + dx).toFixed(6)),
        latitude: Number((cityCenter.latitude + dy).toFixed(6)),
        formattedAddress: `${args.province}${payload.meta.city}${payload.meta.county}`,
      });
    }
  }
  ...
  const industries = countyPayloads.map((payload, index) => {
    const key = `${payload.meta.city}::${payload.meta.county}`;
    const center = countyCenters.get(key) || { longitude: 104.0665, latitude: 30.5728, formattedAddress: `${args.province}${payload.meta.city}${payload.meta.county}` };
    ...
    return {
      ...
      opportunityCount: (payload.opportunityPool || []).length,
      score: Number((county.opportunities || 0) * 10 + (county.documents || 0)).toFixed(0),
    };
  });

  const opportunities = countyPayloads
    .flatMap((payload) => {
      const key = `${payload.meta.city}::${payload.meta.county}`;
      const center = countyCenters.get(key) || { longitude: 104.0665, latitude: 30.5728, formattedAddress: `${args.province}${payload.meta.city}${payload.meta.county}` };
      const county = countyStatsByKey.get(key) || {};
      return (payload.opportunityPool || []).map((item, index) => {
        ...
        return {
          ...
          sourceUrl: item.url || "",
          score,
          battle: pickBattlePlan(item, {
            city: payload.meta.city,
            county: payload.meta.county,
            topDemands: county.topDemands || [],
          }),
        };
      });
    })
    .sort((a, b) => b.score - a.score)
    .slice(0, args.maxOpportunities);
  ...
  const payload = {
    meta: {
      ...
      scopeName: "四川省产业机会地图",
      scopeType: "province",
      centerAddress: "四川省成都市",
      centerLongitude: 104.0665,
      centerLatitude: 30.5728,
      ...
      description: "按区县产业面和重点商机点组织四川省县域产业与招采线索，用于地图展示、商家匹配和销售推进。",
    },
    industries,
    opportunities,
  };
...
main().catch((error) => {
  console.error("[build-sichuan-opportunity-map] failed:", error);
  process.exitCode = 1;
});
