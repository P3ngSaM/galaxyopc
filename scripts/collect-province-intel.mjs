...
function parseArgs(argv) {
  ...
  if (!args.targetsPath) throw new Error("缺少 --targets");
  if (!args.outputDir) throw new Error("缺少 --output-dir");
  return args;
}
...
function toSafeFileSlug(value, fallback = "province") {
  ...
  if (translit[raw]) return translit[raw];
  const cleaned = raw
    .replace(/[省市自治区回族壮族维吾尔特别行政区]/g, "")
    .replace(/[\\/:*?"<>|]/g, "")
    .trim();
...
async function main() {
  ...
  const province = payload.meta?.province || targets[0]?.province || "未知省份";
  const provinceSlug = toSafeFileSlug(province, path.basename(args.outputDir) || "province");
  ...
  const manifestPath = path.join(args.outputDir, `${provinceSlug}-intel-manifest-${new Date().toISOString().slice(0, 10)}.json`);
...
main().catch((error) => {
  console.error("[collect-province-intel] failed:", error);
  process.exitCode = 1;
});
